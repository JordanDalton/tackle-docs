---
description: Register named Cursor automation webhooks, send tasks from Tackle, and inspect delivery history with Tackle Grokbot.
---
...
Source: [JordanDalton/tackle-grokbot](https://github.com/JordanDalton/tackle-grokbot). The installer is available in v0.1.1 and later.
...
1. Run `php artisan grokbot:install` and accept first-bot registration.
2. Name the bot `code-review`, enter its Cursor automation webhook URL, and enter the bearer token at the hidden prompt.
3. Set instructions such as: “Reviews pull requests. Send repository_url, pr_number, and optional review_concerns.”
4. Start `php artisan ai:code` and ask Tackle to send a review task to `code-review`.
5. Inspect delivery history with `php artisan grokbot:history`.

The sending tool accepts a JSON object encoded as a string:

```json
{
  "grokbot_id": 1,
  "payload": "{\"repository_url\":\"https://github.com/example/project\",\"pr_number\":42}"
}
```

The fields inside `payload` must match your automation's instructions. They are an example, not a package-enforced schema.

For unattended production setup after installing the Composer package:

```bash
php artisan grokbot:install --no-interaction --force
```

Register credentials later from an interactive terminal using `php artisan grokbot:register`.
